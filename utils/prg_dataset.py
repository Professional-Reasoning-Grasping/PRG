...
class PRGDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.reason_grasp_data = reason_grasp_data
        ...
        reason_grasp_data, splits = reason_grasp_data.split("|")
        splits = splits.split("_")
        images = []
        for split in splits:
            images_split = glob.glob(
                os.path.join(
                    base_image_dir, "reason_grasp", reason_grasp_data, split, "*.jpg"
                )
            )
            images.extend(images_split)
        jsons = [path.replace(".jpg", ".json") for path in images]
        self.reason_grasp_data = (images, jsons)

        print("number of reason_grasp samples: ", len(images))
    ...
    def __getitem__(self, idx):
        images, jsons = self.reason_grasp_data
        ...
        sampled_grasps = [grasp for _ in range(len(sampled_inds))]
        ...
        if (
            ...
            pos_stack = torch.rand(0, *ori_size)
            cos_stack = torch.rand(0, *ori_size)
            sin_stack = torch.rand(0, *ori_size)
            width_stack = torch.rand(0, *ori_size)

            label = torch.ones(ori_size) * self.ignore_label
        else:


            pos_stack = torch.from_numpy(np.stack([g[0] for g in sampled_grasps], axis=0))
            cos_stack = torch.from_numpy(np.stack([g[1] for g in sampled_grasps], axis=0))
            sin_stack = torch.from_numpy(np.stack([g[2] for g in sampled_grasps], axis=0))
            width_stack = torch.from_numpy(np.stack([g[3] for g in sampled_grasps], axis=0))


            label = torch.ones(pos_stack.shape[1], pos_stack.shape[2]) * self.ignore_label
