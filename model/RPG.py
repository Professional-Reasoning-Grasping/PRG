    ...
    def model_forward(
        ...
        **kwargs,
        ...
        for i in range(len(pred_embeddings)):
            ...
            grasp_output, low_res_masks, iou_predictions = self.model.grasp_decoder(#self.model.visual_model.mask_decoder(
            ...
            pos_output = self.model.visual_model.postprocess_masks(
                grasp_output[0],
                input_size=resize_list[i],
                original_size=label_list[i].shape,
            )

            cos_output = self.model.visual_model.postprocess_masks(
                grasp_output[1],
                input_size=resize_list[i],
                original_size=label_list[i].shape,
            )

            sin_output = self.model.visual_model.postprocess_masks(
                grasp_output[2],
                input_size=resize_list[i],
                original_size=label_list[i].shape,
            )

            width_output = self.model.visual_model.postprocess_masks(
                grasp_output[3],
                input_size=resize_list[i],
                original_size=label_list[i].shape,
            )
        ...
        if inference:
            return {
                "pos_outputs":pos_output_list,
                "cos_outputs":cos_output_list,
                "sin_outputs":sin_output_list,
                "width_outputs":width_output_list,
            }
        ...
        p_loss = self.p_loss_weight * p_loss
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            for i in range(len(pred_embeddings)):
                ...
                sparse_embeddings = sparse_embeddings.to(pred_embeddings[i].dtype)
                grasp_output, low_res_masks, iou_predictions = self.model.grasp_decoder(
                    image_embeddings=image_embeddings[i].unsqueeze(0),
                    image_pe=self.model.visual_model.prompt_encoder.get_dense_pe(),
                    sparse_prompt_embeddings=sparse_embeddings,
                    dense_prompt_embeddings=dense_embeddings,
